...
def hitung_ph(params: Parameter):
    ...
    if params.type_ == "HCl_NaOH":
        pH, status = hitung_asam_kuat_basa_kuat(
            "strongA_strongB", params.c0, v0_liter, params.c_add, params.v_add_ml, Kw
        )
    elif params.type_ == "NaOH_HCl":
        pH, status = hitung_asam_kuat_basa_kuat(
            "strongB_strongA", params.c0, v0_liter, params.c_add, params.v_add_ml, Kw
        )
    else:
        pH, status = hitung_asam_lemah(
            params.c0, v0_liter, params.c_add, params.v_add_ml, params.pKa, Kw
        )
...
fig.add_trace(
    go.Scatter(
        x=vs, y=phs, mode="lines", line=dict(width=4, color="#00ff99"), name="Kurva pH"
    )
)
...
if indicator == "Phenolphthalein":
    fig.add_hrect(
        ...
        fillcolor="pink",
        opacity=0.2,
        annotation_text="Range Phenolphthalein",
    )
elif indicator == "Methyl Orange":
    fig.add_hrect(
        y0=3.1,
        ...
        fillcolor="orange",
        opacity=0.2,
        annotation_text="Range Methyl Orange",
    )
elif indicator == "Bromothymol Blue":
    fig.add_hrect(
        y0=6.0,
        ...
        fillcolor="green",
        opacity=0.2,
        annotation_text="Range Bromothymol Blue",
    )
