...
def hitung_ph(params: Parameter):
    ...
    if params.type_ == "HCl_NaOH":
        pH, status = hitung_asam_kuat_basa_kuat("strongA_strongB", params.c0, params.v0, params.c_add, params.v_add_ml, Kw)
    elif params.type_ == "NaOH_HCl":
        pH, status = hitung_asam_kuat_basa_kuat("strongB_strongA", params.c0, params.v0, params.c_add, params.v_add_ml, Kw)
    else:
        pH, status = hitung_asam_lemah(params.c0, params.v0, params.c_add, params.v_add_ml, params.pKa, Kw)
    pH = max(0.0, min(14.0, pH))
    return pH, status, Kw

# =========================
# FUNGSI WARNA INDIKATOR YANG SESUAI TEORI
# =========================
def get_indicator_color(pH, indicator):
    """
    Mengembalikan warna hex larutan berdasarkan pH dan indikator.
    Sesuai trayek teoritis:
    - Phenolphthalein: tak berwarna (putih) < 8.2, pink 8.2-10, merah muda >10
    - Methyl Orange: merah <3.1, jingga 3.1-4.4, kuning >4.4
    - Bromothymol Blue: kuning <6.0, hijau 6.0-7.6, biru >7.6
    """
    if indicator == "Phenolphthalein":
        if pH < 8.2:
            return "#ffffff"      # tidak berwarna (putih)
        elif pH < 10.0:
            # gradasi pink linear
            ratio = (pH - 8.2) / 1.8
            r = 255
            g = int(200 + (255 - 200) * ratio)  # tetap terang
...
with st.sidebar:
    ...
    # Rekomendasi indikator berdasarkan titrasi
    if type_ == "HCl_NaOH":
        rec_ind = "Bromothymol Blue atau Phenolphthalein"
        warning_ind = None
        if indicator not in ["Bromothymol Blue", "Phenolphthalein"]:
            warning_ind = f"Indikator {indicator} kurang sesuai untuk titrasi ini (perubahan warna tidak terjadi di dekat titik ekuivalen pH 7)."
    elif type_ == "NaOH_HCl":
        rec_ind = "Methyl Orange atau Bromothymol Blue"
        if indicator not in ["Methyl Orange", "Bromothymol Blue"]:
            warning_ind = f"Indikator {indicator} kurang sesuai untuk titrasi ini."
    else:  # CH3COOH_NaOH
        rec_ind = "Phenolphthalein"
        if indicator != "Phenolphthalein":
            warning_ind = f"Untuk titrasi asam lemah-basa kuat, indikator yang tepat adalah Phenolphthalein (perubahan tajam di daerah basa)."
    
    st.info(f"Indikator yang disarankan: {rec_ind}")
    if warning_ind:
        st.warning(warning_ind)
    
    st.caption("Geser slider dan amati perubahan pH serta warna larutan.")

# Parameter objek
params = Parameter(
    type_=type_, temp_c=temp_c, c0=c0, v0=v0,
...
# Area trayek indikator pada kurva
if indicator == "Phenolphthalein":
...
if st.button("Mulai Simulasi Otomatis", type="primary", key="start_auto"):
    ...
    for i, vol in enumerate(volumes):
        tmp_params = Parameter(type_=type_, temp_c=temp_c, c0=c0, v0=v0, c_add=c_add, v_add_ml=float(vol), v_max=v_max, pKa=pKa)
